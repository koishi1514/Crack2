...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    if which_model_netG == 'resnet_9blocks':
        ...
        raise NotImplementedError('Generator output name [%s] is not recognized' % which_model_netG)
    if len(gpu_ids) > 0:
        netG.cuda(gpu_ids[0])
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    if which_model_netD == 'basic':
        ...
        raise NotImplementedError('Discriminator output name [%s] is not recognized' %
                                  which_model_netD)
    if use_gpu:
        netD.cuda(gpu_ids[0])
