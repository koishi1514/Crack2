...
class BaseDataSets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(os.path.join(self.img_data_dir, image_path)).convert('RGB')
        ...
        # axes[0][0].axis('off')
        # axes[0][1].imshow(image.permute((1, 2, 0)),cmap='gray')
        # axes[0][1].axis('off')
        # axes[1][0].imshow(mask1)
        # axes[1][0].axis('off')
        # axes[1][1].imshow(mask.permute((1, 2, 0)),cmap='gray')
        # axes[1][1].axis('off')
        # plt.tight_layout()
        # plt.show()
        # plt.close()
