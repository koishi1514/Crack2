...
class HaarWavelet_1(nn.Module):
    def __init__(self, in_channels):
        ...
        Args:
            in_channels (int): 输入特征图的通道数。
        """
        super(HaarWavelet_1, self).__init__()
        self.haar_transform = HaarWaveletTransform()
        self.in_channels = in_channels

        # 分支 1: 对低频特征进行全局平均池化
        self.branch1 = nn.AdaptiveAvgPool2d(1)  # 对低频特征进行GAP

        # 分支 2: 对高频特征进行全局平均池化
        self.branch2 = nn.AdaptiveAvgPool2d(1)  # 对高频特征进行GAP

        # 全连接层，用于处理每个分支的输出
        self.fc = nn.Sequential(
            nn.Linear(in_channels, in_channels),  # 输入维度是 C
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: 经过 Haar 小波变换和注意力加权后的高频分量。
                          形状为 (batch_size, channels, height/2, width/2)。
        """
        low_f, high_f = self.haar_transform(x)  # 获取低频和高频特征

        # 获取低频特征的分支输出
        branch1_out = self.branch1(low_f)  # 对低频特征进行GAP (B, C, 1, 1)
        branch1_out = branch1_out.view(branch1_out.size(0), -1)  # (B, C)

        # 获取高频特征的分支输出
        branch2_out = self.branch2(high_f)  # 对高频特征进行GAP (B, C, 1, 1)
        branch2_out = branch2_out.view(branch2_out.size(0), -1)  # (B, C)

        # 将branch1和branch2的输出分别输入同一个全连接层
        branch1_fc_out = self.fc(branch1_out)  # (B, C)
        branch2_fc_out = self.fc(branch2_out)  # (B, C)

        # 将两个输出相加
        combined_out = branch1_fc_out + branch2_fc_out  # (B, C)

        # 使用 Sigmoid 激活函数得到最终的注意力权重
        attention_weights = self.sigmoid(combined_out).unsqueeze(-1).unsqueeze(-1)  # (B, C, 1, 1)

        # 最终输出：将高频特征加权后与低频特征相加
        high_f = high_f * attention_weights
        low_f = low_f
