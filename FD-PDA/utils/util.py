...
def load_model(path):
    """Loads output and return it without DataParallel table."""
...
def load_checkpoint(path, model, optimizer, from_ddp=False):
    ...
    Args:
        ...
        model (output): output to restore checkpoint to
        optimizer (optimizer): torch optimizer to load optimizer state_dict to
        from_ddp (bool, optional): load DistributedDataParallel checkpoint to regular output. Defaults to False.

    Returns:
        output, optimizer, epoch_num, loss
    """
    # load checkpoint
    checkpoint = torch.load(path)
    # transfer state_dict from checkpoint to output
    model.load_state_dict(checkpoint["state_dict"])
    # transfer optimizer state_dict from checkpoint to output
...
def restore_model(logger, snapshot_path, model_num=None):
    ...
    Returns:
        output, optimizer, start_epoch, performance
    """
    try:
    ...
    try:
        logger.info(f"Restoring output checkpoint: {model_checkpoint}")
        ...
        logger.warning(f"Unable to restore output checkpoint: {e}, using new output")


def save_checkpoint(epoch, model, optimizer, loss, path):
    """Saves output as checkpoint"""
    torch.save(
        {
            "epoch": epoch,
